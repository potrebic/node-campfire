...
exports.createCampfireAPI = function (options) {
     ...
     * @param {function} callback (error, request, response, body)
     */
    that.user = function(userId, callback) {
        var uri = composeUri('users/{0}', userId);
        execute(getter(uri), callback);
    };

    /**
     * Get info on user making the request
     *
     * @param {function} callback (error, request, response, body)
     */
    that.me = function(callback) {
        var uri = composeUri('users/me');
        execute(getter(uri), callback);
    };
